...
# -----------------------
GEMINI_API_KEYS = [
    "AIzaSyDY63c35vh3rmBFeFGQcP761TKFRTEang4",  # Key 1
    "AIzaSyATPdstRR41HPr6KyqZgvQpwFJDbKyppKg",  # Key 2
...
# -----------------------
def generate_with_fallback(prompt: str, model_name="models/gemini-2.5-flash"):
    last_exception = None
    for key in GEMINI_API_KEYS:
        try:
            ...
            # Check if response indicates quota hit
            if "quota" in text.lower() or "429" in text:
                print(f"Quota hit with key {key[:6]}***. Trying next key...")
                continue

            print(f"Used API key: {key[:6]}*** successfully")
            return text
...
# -----------------------
app = Flask(__name__)
app.secret_key = "a_very_secret_key_for_your_project"
